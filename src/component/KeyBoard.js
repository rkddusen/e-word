...
function KeyBoard(props) {
  ...
  useEffect(() => {
    if (props.word && props.result) {
      ...
      for (let i = 1; i < 5; i++) {
        for (let j = 0; j < _arrWord.length; j++) {
          if (props.word[i] === _arrWord[j]) {
            // 중복 값이 있을 때
            isin = true;
            if (_arrResult[j] === 'O' && ( props.result[i] === 'B' || props.result[i] === 'S')) {
              if(_arrResult[j] === 'B' &&  props.result[i] === 'S')
                // 기존 결과보다 더 정확한 결과
      ...
      if (key.length < 1) { // key에 아무것도 없을 때
        console.log('시작')
        setKey(_arrWord);
        setValue(_arrResult);
      } else { // key와의 중복 체크
        ...
        for (let i = 0; i < _arrWord.length; i++) {
          ...
          if(index !== -1){ // 중복 있음
            if(value[index] ==='O'){
              if(_arrResult[i] === 'S' || _arrResult[i] === 'B'){ // 새로 추가되는 결과가 더 좋을 때
                value[index] = _arrResult[i];
              }
            } else if(value[index] ==='B'){
              if(_arrResult[i] === 'S'){ // 새로 추가되는 결과가 더 좋을 때
                value[index] = _arrResult[i];
              }
            }
          } else { // 중복 없음
